...
- 🎙️ **O que mudou:** o dono de um app de delivery chega com a ideia inicial: *"Por enquanto, só quero **cadastrar e listar os restaurantes e os clientes**. Deixa o resto pra depois — quero ver algo funcionando logo."* Ainda não há regras; é o primeiro esboço do sistema.
- 💡 **Conceito da aula:** variáveis, listas, laços, entrada/saída (ainda **sem classes**).
- 🎯 **O que fazer no seu projeto:** façam um programa de menu que permita **adicionar** e **listar** restaurantes, guardando tudo numa lista/array dentro do `main`. Só tipos básicos.
- 📦 **Entregável:** programa Java que roda e mostra a lista de restaurantes cadastrados.
...
- 🎙️ **O que mudou:** na segunda conversa, o dono de um app de delivery completa: *"Ah, esqueci — também preciso registrar os pedidos dos clientes, e cada pedido tem vários itens."* De repente o programa do Encontro 1 fica confuso: os mesmos dados se repetem e se espalham por várias listas.
- 💡 **Conceito da aula:** objeto = estado + comportamento; como identificar entidades.
- 🎯 **O que fazer no seu projeto:** releiam o briefing + esse novo pedido e **grifem**: cada substantivo importante (restaurante, pedido…) vira candidato a **classe**; cada característica, a **atributo**; cada verbo, a **operação**. Montem a tabela "termo → classe / atributo / operação".
...
- 🎙️ **O que mudou:** o **gerente do projeto** entra na conversa: *"O cliente falou um monte de coisa na reunião — até o time de futebol do restaurante. Nem tudo importa. Foquem no que o sistema precisa de verdade."*
...
- 🎯 **O que fazer no seu projeto:** transformem as classes candidatas do Encontro 3 em **classes Java**, cada uma com **construtor** que exige os dados obrigatórios. No `main`, criem alguns objetos (ex.: alguns restaurantes) e usem.
...
- 🎙️ **O que mudou:** conversando de novo, o dono de um app de delivery solta uma regra que ninguém tinha dito: *"Tem uma coisa que não pode acontecer de jeito nenhum: não pode atribuir dois entregadores ao mesmo pedido."* Isso não é um dado — é um **comportamento** que o sistema precisa garantir.
- 💡 **Conceito da aula:** atributos (visibilidade) e operações (assinatura).
- 🎯 **O que fazer no seu projeto:** definam a **visibilidade** de cada atributo e a **assinatura** de cada operação (parâmetros e retorno). Decidam **qual classe** será dona da regra "não pode atribuir dois entregadores ao mesmo pedido" e criem a operação que a implementa.
...
- 🎙️ **O que mudou:** o **dev sênior** chega preocupado com um incidente: *"Tivemos um problema sério: a taxa de entrega ficou negativa. Alguém alterou o dado direto, sem passar por validação. Não pode se repetir."*
- 💡 **Conceito da aula:** `private` + validação; proteger o invariante.
- 🎯 **O que fazer no seu projeto:** deixem os atributos `private` e criem métodos que **validam antes de alterar** o estado. Implementem em código a regra "não pode atribuir dois entregadores ao mesmo pedido": se a operação a violaria, ela **recusa** (lança exceção) em vez de gravar o valor errado.
...
- 🎙️ **O que mudou:** o **analista** aponta: *"Precisamos enxergar como as coisas se conectam — um pedido é de um cliente e um restaurante, e é levado por um entregador — e quantas de cada lado."* As classes deixam de ser ilhas.
- 💡 **Conceito da aula:** associação, multiplicidade, agregação × composição.
- 🎯 **O que fazer no seu projeto:** liguem suas classes com a **multiplicidade nas duas pontas** (um pedido é de um cliente e um restaurante, e é levado por um entregador). Para cada "tem-um", decidam: a parte **morre com o todo** (composição ◆) ou **sobrevive** (agregação ◇)?
...
- 🎙️ **O que mudou:** o dono de um app de delivery traz uma novidade de negócio: *"Passamos a ter um **cliente assinante**. É quase igual aos outros clientes, mas com um detalhe: tem entrega grátis acima de um valor."* Copiar e colar a classe inteira só pra mudar um detalhe seria desperdício (e fonte de bug).
- 💡 **Conceito da aula:** `extends`, `super`, reúso (o "é um").
- 🎯 **O que fazer no seu projeto:** vocês já têm a classe **`cliente`**. Criem uma **subclasse** para o **cliente assinante** que **herda** de `cliente` e **acrescenta só o que muda** (tem entrega grátis acima de um valor) — não reescrevam o que já existe, reaproveitem com `super`. No `main`, criem um cliente assinante e usem-no onde um `cliente` é esperado.
- 📦 **Entregável:** subclasse de `cliente` funcionando, **sem duplicar** o código da base.
- 🔒 **Restrição nova:** só herdem se passar no teste **"é um"** (cliente assinante **é um** cliente?); se for "tem um", usem composição. A superclasse pode ser `abstract`.
...
- 🎙️ **O que mudou:** o **dev sênior** sente um cheiro ruim no código: *"Vi vários `if` verificando o tipo do cliente pra tratar o cliente assinante de um jeito diferente. Isso vira um pesadelo quando aparecer mais um tipo."*
- 💡 **Conceito da aula:** sobrescrita (`@Override`), ligação dinâmica.
- 🎯 **O que fazer no seu projeto:** peguem o comportamento que muda entre os tipos (ex.: tem entrega grátis acima de um valor) e **sobrescrevam** esse método na subclasse cliente assinante. No resto do sistema, chamem o método pela **referência de `cliente`** e deixem o objeto decidir o que fazer.
...
- 🎯 **O que fazer no seu projeto:** identifiquem os **atores** (quem usa o sistema) e listem os **casos de uso** (verbo + objeto). Desenhem o diagrama e escrevam o **fluxo principal + 1 alternativo** do caso mais importante (fazer um pedido até a entrega).
...
- 🎙️ **O que mudou:** o **dev sênior** antecipa uma mudança: *"Hoje só temos um jeito de lidar com a forma de pagamento (na entrega ou online), mas isso vai mudar em breve. Não podem amarrar o sistema a uma implementação só."*
- 💡 **Conceito da aula:** classe abstrata + interface (contrato).
- 🎯 **O que fazer no seu projeto:** criem a interface **`MeioDePagamento`** para a forma de pagamento (na entrega ou online), com pelo menos **duas implementações** diferentes. Façam o resto do sistema depender **só da interface** e escolher a implementação na hora de criar o objeto.
...
- 🎙️ **O que mudou:** o **analista de qualidade** quer validar o comportamento: *"Me mostra, passo a passo, o que acontece quando alguém vai fazer um pedido até a entrega. Quero ver **quem chama quem** e o que acontece se algo der errado."*
- 💡 **Conceito da aula:** objetos concretos + mensagens no tempo.
- 🎯 **O que fazer no seu projeto:** desenhem a **sequência** de "fazer um pedido até a entrega", mostrando as mensagens entre os objetos na ordem em que acontecem, incluindo o **caminho alternativo** (por ex., quando a regra "não pode atribuir dois entregadores ao mesmo pedido" é violada).
...
- 🎯 **O que fazer no seu projeto:** mapeiem os estados de o pedido (`recebido → em preparo → saiu para entrega → entregue; ou cancelado`) e o evento que dispara cada transição. Depois, no código, garantam que só dá para ir de um estado ao **próximo válido**.
...
- 🎙️ **O que mudou:** o dono de um app de delivery quer treinar a equipe nova: *"Preciso do **passo a passo do pedido do cliente à entrega na porta**, de um jeito que qualquer funcionário entenda o processo."*
- 💡 **Conceito da aula:** fluxo de processo, decisões, raias.
- 🎯 **O que fazer no seu projeto:** modelem o processo do pedido do cliente à entrega na porta com início, ações, **decisões** e fim; usem **raias** para deixar claro **quem** faz cada passo (cliente, atendente, sistema).
...
- 🎙️ **O que mudou:** o **tech lead** dá o alerta numa revisão: *"O sistema cresceu e virou uma bola de lama — toda mudancinha quebra outra coisa. Por exemplo: a classe Pedido calcula o total, escolhe o entregador E notifica. Precisamos organizar."*
- 💡 **Conceito da aula:** SRP (responsabilidade única), OCP (aberto/fechado)…
- 🎯 **O que fazer no seu projeto:** achem a classe que "faz demais" e **separem responsabilidades** — comecem por: a classe Pedido calcula o total, escolhe o entregador E notifica. Deixem a **regra de negócio** numa classe e a parte de **tela/arquivo/e-mail** em outra.
...
- 🎙️ **O que mudou:** o **tech lead** revisa de novo: *"Agora exageraram pro outro lado — a lógica de taxa de entrega por distância cheia de casos — e ainda tem código que ninguém usa. **Simplifiquem.**"*
- 💡 **Conceito da aula:** KISS (simplicidade), DRY (não repita), YAGNI (não faça o que não pediram).
- 🎯 **O que fazer no seu projeto:** removam duplicação (extraiam método), simplifiquem a lógica de taxa de entrega por distância cheia de casos e **apaguem** todo código que nenhum requisito atual pede.
...
- 🎙️ **O que mudou:** o **arquiteto** planeja crescer o time: *"Outro time vai assumir a parte de pedidos e logística de entrega. Precisamos separar isso em **módulos** com fronteiras claras."*
- 💡 **Conceito da aula:** componentes (interfaces) + pacotes (camadas, sem ciclos).
- 🎯 **O que fazer no seu projeto:** agrupem as classes em **pacotes por responsabilidade** e reorganizem o código em `com.delivery.{dominio,aplicacao,infra}`. Desenhem as dependências entre os pacotes.
...
- 🎙️ **O que mudou:** o pessoal de **infra (DevOps)** entra na conversa: *"Antes de publicar, preciso saber **onde** isso vai rodar. Pensei em o app do cliente, o app do entregador, um servidor e um banco. Como as partes se conectam?"*
- 💡 **Conceito da aula:** nós (hardware/ambiente), artefatos, conexões de rede.
- 🎯 **O que fazer no seu projeto:** desenhem onde cada parte roda (o app do cliente, o app do entregador, um servidor e um banco) e rotulem o **protocolo** de cada conexão (HTTP, JDBC…). Marquem o que é **público** e o que é **interno**.
...
- 🎙️ **O que mudou:** o dono de um app de delivery e o **gerente** fecham o ciclo: *"Chegou a hora: quero ver **rodando de ponta a ponta** — fazer um pedido até a entrega — do jeito que a gente combinou."*
- 💡 **Conceito da aula:** integração + um padrão simples (Factory, Strategy).
- 🎯 **O que fazer no seu projeto:** montem um `Principal` que executa "fazer um pedido até a entrega" do começo ao fim, usando as classes de verdade. Se um padrão simples ajudar (ex.: *Factory* para criar objetos, *Strategy* para a forma de pagamento (na entrega ou online)), apliquem — **sem forçar**.
